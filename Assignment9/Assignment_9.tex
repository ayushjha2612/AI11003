...
\item
To find $ \Pr\brak{ X = Y } $, we use the conditional pdf,
\begin{align}
    &f_{X|Y}\brak{x|y} = \frac{ f_{XY}\brak{x,y}}{f_Y(y)} \\
    &= \lim_{\Delta x, \Delta y  \to 0} \Pr\brak{x < X \le x+ \Delta x | y < Y \le y+ \Delta y} \\
    &\approx  \frac{\Pr\brak{X=x ,Y= y}}{\Pr\brak{X=x}}
\end{align}
Similarly we have,
\begin{align}
     f_{Y|X}\brak{y|x} &= \frac{ f_{XY}\brak{x,y}}{f_X(x)} \\
    &\approx  \frac{\Pr\brak{X=x ,Y= y}}{\Pr\brak{Y=y}}
\end{align}
\begin{align}
\label{eq:: joint_pdf1}
f_{XY}(x,y)  = 
...
\end{cases}
\end{align}
Calculation of conditional PDF and using \eqref{eq:: joint_pdf1},
\begin{align}
    f_{X|Y}\brak{x|y} &= \frac{ f_{XY}\brak{x,y}}{f_Y(y)} \\
    &= \frac{0}{f_Y(y)} \text{ for } x=y \\
    &= 0 \text{ for } x=y 
\end{align}
Similarly we have,
\begin{align}
     f_{Y|X}\brak{y|x} &= 0  \text{ for } x=y \\
\end{align}
Therefore for $x=y $we have,
\begin{align}
       & f_{X|Y}\brak{x|y} = 0 \\
       & f_{Y|X}\brak{y|x} = 0 \\
       & \implies\frac{\Pr\brak{X=x ,Y= y}}{\Pr\brak{Y=y}} = 0  \\
       & \Pr\brak{X=x ,Y= y} = 0 \text{ for } x=y  \\
       & \implies \Pr\brak{X=Y} = 0 
\end{align}
