...
\begin{align}
  F_{Xy}(x,y) &= \Pr\brak{X \le x, Y \le y} \\
  &=  \Int_{-\infty}^{y} \Int_{-\infty}^{x} f_{XY}(u,v) \,du\,dv  \\
  &= \Int_{x}^{y} \Int_{0}^{x} 4uv \,du\,dv  \\
  &=  \int_{x}^{y} 2x^2v \,dv \\
  & = x^2v^2  \Biggr|_{x}^{y} \\
  &= x^2(y^2 - x^2)
\end{align}
...
Therefore, 
\begin{align}
 F_{Xy}(x,x) &=  \Pr\brak{X \le x, Y \le x}
  &=  x^2(x^2 - x^2)\\
   &= 0
\end{align}
This implies,
\begin{align}
 \Pr\brak{X \le x, Y \le x}  =  0 \\
